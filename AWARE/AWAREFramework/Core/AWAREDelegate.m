...
- (BOOL)application:(UIApplication *)application didFinishLaunchingWithOptions:(NSDictionary *)launchOptions {
    ...
    // EntityCall *entity = [NSEntityDescription insertNewObjectForEntityForName:NSStringFromClass([EntityCall class])
...
static NSString *dataBaseName = @"AWARE.sqlite";
...
- (NSPersistentStoreCoordinator *)persistentStoreCoordinator {
    ...
    __persistentStoreCoordinator = [EncryptedStore makeStoreWithOptions:@{
                                                         ...
                                                         EncryptedStoreDatabaseLocation : [storeURL description],
    ...
    NSPersistentStore *persistenceStore = [__persistentStoreCoordinator addPersistentStoreWithType:NSSQLiteStoreType
                                                                                     configuration:nil
                                                                                               URL:storeURL
                                                                                           options:options
                                                                                             error:&error];
    ...
    if (persistenceStore != nil && error == nil) {
        NSLog(@"Instance DB initialized success");
    } else {
        NSLog(@"There's an error creating the database %@", [error userInfo]);
    }
...
- (int) fetchNumberOfRecordsWithEntityName:(NSString *)entityName {
    ...
    if(entityName == nil) return 0;
    
    NSManagedObjectContext *moc = __managedObjectContext;
    NSFetchRequest *request = [NSFetchRequest fetchRequestWithEntityName:entityName];
    
    NSError *error = nil;
    long count = [moc countForFetchRequest:request error:&error];
    return (int)count;
}
